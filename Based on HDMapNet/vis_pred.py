...
def vis_segmentation(model, val_loader):
    ...
    with torch.no_grad():
        for batchi, (
        imgs, trans, rots, intrins, post_trans, post_rots, lidar_data, lidar_mask, car_trans, yaw_pitch_roll,
        semantic_gt, instance_gt, direction_gt) in enumerate(val_loader):
...
def vis_vector(model, val_loader, angle_class):
    ...
    with torch.no_grad():
        for batchi, (
        imgs, trans, rots, intrins, post_trans, post_rots, lidar_data, lidar_mask, car_trans, yaw_pitch_roll,
        segmentation_gt, instance_gt, direction_gt) in enumerate(val_loader):
            ...
            # Process the outputs if they are valid
            for si in range(segmentation.shape[0]):
                ...
                plt.imshow(car_img, extent=[segmentation.shape[3] // 2 - 15, segmentation.shape[3] // 2 + 15,
                                            segmentation.shape[2] // 2 - 12, segmentation.shape[2] // 2 + 12])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--modelf', type=str, default='runs/model.pt')
